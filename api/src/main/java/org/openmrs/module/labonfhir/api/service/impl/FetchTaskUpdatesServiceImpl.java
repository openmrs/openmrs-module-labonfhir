...
@Component("fetchTaskUpdatesServiceImpl")
@Setter(AccessLevel.PACKAGE)
public class FetchTaskUpdatesServiceImpl implements FetchTaskUpdatesService {
	
	private static Log log = LogFactory.getLog(FetchTaskUpdatesServiceImpl.class);
			...
			if (uuid != null) {
				if (userExists) {
					taskBundle = fhirConfig.getFhirClient().search().forResource(Task.class)
					        .where(Task.RES_ID.exactly().code(uuid.toString())).where(Task.OWNER.hasId(practitionerId))
					...
					taskBundle = fhirConfig.getFhirClient().search().forResource(Task.class)
					        .where(Task.RES_ID.exactly().code(uuid.toString()))
					        .where(Task.STATUS.exactly().codes(TaskStatus.COMPLETED.toCode(), TaskStatus.ACCEPTED.toCode(),
					            TaskStatus.REJECTED.toCode(), TaskStatus.CANCELLED.toCode()))
					        .lastUpdated(lastUpdated).returnBundle(Bundle.class).execute();
